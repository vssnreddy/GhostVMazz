...
Player.prototype.checkByEnemy = function () {
    for (var i = 0; i < enemies.length; i++) {
        if (collision2p(this , enemies[i]) && !enemies[i].isDead) {
        	document.getElementById('ab').play();
        	alert("you are dead!! " + (Ncount * 5 + Ecount) + " kills");
        	location.reload();
        }
    }
};
...
Enemy.prototype.die = function () {
    ...
    if(Ncount == 5)
    {
        window.alert("you are the winner!! " + (Ncount * 5 + Ecount) + " kills");
    	location.reload();
    }
...
function collision2p(a, b) {
    return a.centerX <= b.drawX + b.width &&
        a.centerX >= b.drawX &&
        a.centerY <= b.drawY + b.height &&
        a.centerY >= b.drawY;
}